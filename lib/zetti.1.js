...
function loadup(t, elmnt, forMDE) {
  ...
  if (fname.endsWith('.md')) {
    document.getElementById('medit').style.display = "initial"; //show menu item for markdown file edit
    if (local || forMDE) {
    ...
    if (!loadOriginal || forMDE) {
      ...
      html = `<iframe id='ed' style='display:none' src='./tinymde.html?${fname}${o}' width="100%" allowfullscreen scrolling="no"></iframe><!-- start -->` + html;
      //   html+=`<!-- end -->
      // <button onclick="let ed=document.getElementById('ed');if (ed.style.display=='none'){ed.style.display='block';resizeiframe(ed);} else {loadfile(fname,main,false);}">Toggle ed</button>
      // `;
    }
